...
- Pricing: public entry prices on `/geo-optimierung/` and `/en/geo-optimization/` for the three commercial offers (GEO Audit from 1.500 EUR net, Seiten-Sprint from 2.500 EUR net, laufende SEO- und GEO-Betreuung from 1.250 EUR net per month on `/seo-betreuung/`). Always show `ab`/`from`, `netto`/`net`, and `/ Monat` where monthly. These are entry prices for bounded scopes; the binding quote follows scope agreement. Do not publish discounts, invented savings, or unfounded `most popular` badges, and do not replace a price with `auf Anfrage` alone.
- Public contact email: kontakt@paternoga-seo-geo.de.
- Core claim: `Be the brand AI thinks of first.`
- Revenue Sprint 1 implementation, attribution boundaries, regression tests and release evidence: `docs/revenue-sprint-1-2026-09-06.md`. Audit and page sprint are direct inquiry offers; research and Dresden remain separate later work. Its inquiry-only pricing rule was superseded on 2026-09-06 by the public entry prices above — do not restore `auf Anfrage` as the only price from that older document.
- `src/content/offers.ts` is the single source of truth for the three commercial offers: entry price, base scope, deliverables, duration, limits and CTA. The GEO hub package cards, the visible `OfferScope` block on each owner route and the JSON-LD `Service.offers` all read from it. Never publish a price, scope or duration that is not visible on the page itself, and never let structured data assert more than the visible copy.
- Offer facts must stay explicitly labelled and machine-extractable (price, suited to, scope, deliverables, duration, limits, next step). Do not bury them in marketing paragraphs. Do not claim that PATERNOGA services appear in ChatGPT Shopping, Instant Checkout or comparable commerce surfaces; those are product/retail oriented.
...
- 2026-09-06: Revenue Sprint 3 retainer migration. The ongoing-support offer moved from `/geo-betreuung/` to `/seo-betreuung/` (EN `/en/geo-support/` to `/en/seo-support/`) with single-hop 301s, because "SEO Betreuung" is the established commercial retainer intent while "GEO Betreuung" is not a standalone category. Do not restore the old URLs and do not create a second parallel retainer page. `offer_type: geo_support` stays unchanged internally so measurement history is not broken. Reasoning: `docs/revenue-sprint-3-retainer-growth-2026-09-06.md`.
- 2026-09-06: No local retainer route. `/seo-betreuung-dresden/` was evaluated and rejected — no standalone SERP; the intent is covered inside `/seo-dresden/`.
- 2026-09-06: Revenue Sprint 1.5 positioning decision. PATERNOGA is an SEO and GEO studio for companies whose offer needs explaining before anyone buys; the unit of work is the small number of pages their enquiries come from, checked separately against three readers (search engine, answer system, deciding human). The page is the unit of work, not the category name. Buyer fit is defined by buyer job, page structure and a decision-authorised contact, explicitly not by ownership structure or number of services. Full reasoning, red team and rejected variants: `docs/seo-positioning-sprint-1-5-2026-09-06.md`.
- 2026-09-06: Public entry prices introduced on the GEO hub for the three separated offers, replacing the previous inquiry-only rule. The four purchase paths stay separate: free technical check, GEO Audit, Seiten-Sprint, GEO-Betreuung.
- 2026-09-06: SEO and GEO are described as sharing technical and editorial foundations. Never state that ChatGPT, Perplexity or comparable systems depend on the Google index; they operate their own crawlers and retrieval paths.
