  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
      if (!document.hidden && !reducedMotionQuery.matches) {
        elapsedTime += (time - lastFrameTime) * 0.001 * speedRef.current;
      }
      lastFrameTime = time;
      ...
      (canvas as HTMLCanvasElement & { __liquidFrame?: number }).__liquidFrame = ++renderedFrameCount;
      animationFrameId = reducedMotionQuery.matches || document.hidden || !isVisible ? 0 : requestAnimationFrame(render);
    };

    const startAnimation = () => {
      if (animationFrameId || document.hidden || reducedMotionQuery.matches || !isVisible) return;
    ...
    const handleReducedMotionChange = () => {
      stopAnimation();
      if (reducedMotionQuery.matches) render(performance.now());
      else startAnimation();
    };

    window.addEventListener("resize", resize);
    if (interactiveRef.current) window.addEventListener("mousemove", handleMouseMove);
    ...
    return () => {
      ...
      if (interactiveRef.current) window.removeEventListener("mousemove", handleMouseMove);
